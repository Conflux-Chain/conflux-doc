...
At the beginning, we should get the bytecode and ABI of the smart contract we want to deploy. Here we use Hash Time Locked Contract(HTLC):
```js
...
```
Add a wallet account using your private key:
```js
...
```
Get current nonce of your account:
```js
let nonce = await confluxWeb.cfx.getTransactionCount(pub_key);
```
Set transaction parameters with nonce and contract bytecode, leaving field *to* empty denotes this is a contract creation:
```js
const txParams = {
...
```
Sign this transaction and get raw transaction string:
```js
...
let contract_addr = receipt.contractCreated;
```
...
```
Same as above, we get the current nonce and build a transaction with the ABI of the contract method we want to call, then
send it and wait for its receipt.
```js
let nonce = await confluxWeb.cfx.getTransactionCount(pub_key);
// transaction params
const txParams = {
...
// sign, send and wait for receiptconst {rawTransaction} = await confluxWeb.cfx.signTransaction(txParams);
const tx_hash = await confluxWeb.cfx.sendSignedTransaction(rawTransaction);
const receipt = await waitForReceipt(tx_hash);
```
